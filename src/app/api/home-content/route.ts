...
const CMS_HOME_ENDPOINT = 'https://cms.confitechone.com/wp-json/wp/v2/pages?slug=home&_fields=acf';
...
const pickButtonLink = (acf: ACFPayload) => {
  const directLink = pickFirst(acf, ['button_link', 'hero_button_link', 'link']);
...
export async function GET() {
  try {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to fetch home content' }, { status: response.status });
    }
    ...
    return NextResponse.json({ error: 'Unexpected server error while fetching home content' }, { status: 500 });
  }
}
