...
} from "../../members/constants/followupEmail";
...
} from "../../members/constants/emailSignature";
import { EmailSignatureSection } from "@/components/features/emailSignature";
import { EmailSignaturePreview } from "@/components/features/emailSignature/Emailsignaturepreview";
import { AccessoriesSection } from "@/components/features/accessiories";
...
function ActivePanel({
  ...
  emailSignatureForm: EmailSignatureFormData;
  onEmailSignatureChange: (p: Partial<EmailSignatureFormData>) => void;
  memberName: string;
  cardUrl: string;
}) {
  switch (id) {
    ...
    case "accessories":
      return (
        <AccessoriesSection memberName={memberName} activationUrl={cardUrl} />
      );
  }
}
...
function RightPreviewPanel({
  ...
  if (activeSection === "email-signature") {
    return (
      <EmailSignaturePreview
        form={emailSignatureForm}
        onSaveToEmail={() => console.log("Save to email")}
...
export default function MemberProfilePage({ memberId }: { memberId: string }) {
  ...
  const [emailSignatureForm, setEmailSignatureForm] =
    useState<EmailSignatureFormData>(DEFAULT_EMAIL_SIGNATURE_FORM);

  const memberName = form.name || "Diago";
      ...
      >
        ...
        >
          <ActivePanel
            ...
            emailSignatureForm={emailSignatureForm}
            onEmailSignatureChange={(p) =>
              setEmailSignatureForm((prev) => ({ ...prev, ...p }))
            }
